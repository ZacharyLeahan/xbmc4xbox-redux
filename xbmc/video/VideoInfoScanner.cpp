...
namespace VIDEO
{
  ...
  void CVideoInfoScanner::GetArtwork(CFileItem *pItem, const CONTENT_TYPE &content, bool bApplyToDir, bool useLocal, const std::string &actorArtPath)
  {
    ...
    // add online art
    for (std::vector<CScraperUrl::SUrlEntry>::const_iterator it = pItem->GetVideoInfoTag()->m_strPictureURL.m_url.begin(); it != pItem->GetVideoInfoTag()->m_strPictureURL.m_url.end(); ++it)
    {
      const CScraperUrl::SUrlEntry &i = *it;
      ...
      std::map<int, std::map<std::string, std::string> > &seasonArt, const std::vector<std::string> &artTypes, bool useLocal)
  {
    ...
    // add online art
    for (std::vector<CScraperUrl::SUrlEntry>::const_iterator it = show.m_strPictureURL.m_url.begin(); it != show.m_strPictureURL.m_url.end(); ++it)
    {
      const CScraperUrl::SUrlEntry &i = *it;

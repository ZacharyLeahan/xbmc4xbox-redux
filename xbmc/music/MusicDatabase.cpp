...
bool CMusicDatabase::AddAlbum(CAlbum& album)
{
  ...
  for (VECSONGS::const_iterator it = album.infoSongs.begin(); it != album.infoSongs.end(); ++it)
    AddAlbumInfoSong(album.idAlbum, *it);
  ...
  try
  {
    ...
    { // try and ascertain scraper for this path
      ...
      { // use default scraper of the requested type
        ...
        {
          info = boost::dynamic_pointer_cast<ADDON::CScraper>(defaultScraper);
        }
      }
    }
    m_pDS->close();
